...
.eqv IMG_HEIGHT		240
.eqv IMG_SIZE 		230400	# 320 x 240 x 3 (pixels)
...
imgInfo: .space		12	# image descriptor memory allocation
...
# predefined header data
# note: bytes of each word in the header_data array are read little-endian (even though words are read left to right)
header_data: .word 0x84364d42, 0x00000003, 0x00360000, 0x00280000, 0x01400000, 0x00f00000, 0x00010000, 0x00000018, 0x84000000, 0x00000003, 0x00000000, 0x00000000, 0x00000000, 0x0000


file_name: .asciz "shading.bmp"
...
# return value:
#	a0 - 0 if no errors, -1 (error) otherwise
# notes:
#	there is no need to pad the file as the image width in bytes is already divisible by 4
save_file:
	
	mv t0, a0			# save imgInfo address for future use
	li a7, system_OpenFile
        lw a0, ImgInfo_fname(t0)	# file name 
        li a1, 1			# flags: 1-write file
        ecall
	
	blt a0, zero, sf_error		# branch to sf_error if error code
	mv t1, a0			# save file handle for the future
	
	li a7, system_WriteFile
	lw a1, ImgInfo_hdrdat(t0)	# pointer to bmp header
	li a2, BMPHeader_Size		# write header
	ecall
	
	li a7, system_WriteFile
	mv a0, t1			# read header 
	li a2, IMG_SIZE
	lw a1, ImgInfo_imdat(t0)	# pointer to image data array
	ecall
...
modify_file:
	...
	lw s5, ImgInfo_imdat(a0)	# address of image data
